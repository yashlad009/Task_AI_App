...
function AnimatedBar({ count, maxCount, isToday, index }: { count: number; maxCount: number; isToday: boolean; index: number }) {
...
function AnimatedBar2({ percent, color = '#6C63FF', delay = 300 }: { percent: number; color?: string; delay?: number }) {
  ...
  useEffect(() => { w.value = withTiming(percent, { duration: 900 }); }, [percent]);
  const barStyle = useAnimatedStyle(() => ({
    width: `${interpolate(w.value, [0, 100], [0, 100], Extrapolation.CLAMP)}%`,
  }));
  return (
    <View style={styles.rateBg}>
      <Animated.View style={[styles.rateFill, barStyle, { backgroundColor: color }]} />
    </View>
  );
}

// Stat card with entering animation
function StatCard({ icon, value, label, color, index }: { icon: any; value: string | number; label: string; color: string; index: number }) {
  return (
    <Animated.View entering={FadeInDown.delay(index * 70).duration(400).springify()} style={[styles.statCard, { borderColor: color }]}>
  ...
  return (
    ...
    >
      ...
      <View style={styles.statsGrid}>
        <StatCard icon="clipboard-outline"        value={data?.total ?? 0}               label="Total Tasks" color="#6C63FF" index={0} />
        <StatCard icon="checkmark-circle-outline" value={data?.completed ?? 0}           label="Completed"   color="#22C55E" index={1} />
        <StatCard icon="time-outline"             value={data?.pending ?? 0}             label="Pending"     color="#F59E0B" index={2} />
        <StatCard icon="flash-outline"            value={`${data?.efficiency ?? 0}%`}   label="Efficiency"  color="#EC4899" index={3} />
      </View>
          ...
          {weeklyData.map((d, i) => (
            <View key={i} style={styles.chartCol}>
              ...
              <View style={styles.barContainer}>
                <AnimatedBar count={d.count} maxCount={maxCount} isToday={d.isToday} index={i} />
              </View>
      ...
      <Animated.View entering={FadeInUp.delay(400).duration(400)} style={styles.card}>
        ...
        <View style={styles.achieveGrid}>
          {g?.achievements.map((a, i) => <AchievementCard key={a.key} a={a} index={i} />)}
        </View>
      </Animated.View>

      {/* Category tokens */}
      <Animated.View entering={FadeInUp.delay(450).duration(400)} style={[styles.card, { marginBottom: 32 }]}>
